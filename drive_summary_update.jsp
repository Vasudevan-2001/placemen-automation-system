...
<%
  ...
  if(!sessemail.equals(""))
  {
      ...
      drive_stat=request.getParameter("drive_stat");
    

      
      if(drive_stat.equals("ATTENDED")){
           update="update tbl_drive set drive_status='"+drive_stat+"' where id='"+cid+"' and regno='"+regno+"' and dept='"+dept+"' and yop='"+yop+"'";
           res=stmt.executeUpdate(update);
            if(res>0){%>
                <script>alert('drive_status updated');location.href='drive_summary.jsp?cid=<%=cid%>&dept=<%=dept%>&yop=<%=yop%>';</script>
            <%}
      }
      else{
            update1="update tbl_drive set drive_status='"+drive_stat+"' where id='"+cid+"' and regno='"+regno+"' and dept='"+dept+"' and yop='"+yop+"'";
